...
int numero_interacoes=12500; // total de interações
double delta_t=1; 


void inicializa_VW(){  // aqui inicializamos V e as condições de contorno 
 	int i,j;
 	for(i = 0; i <= I+1; i++){
 		for (j = 0; j <= J+1; j++){
 			W[i][j] = 0.0;
...
void novo_V(double Vmp[I+2][J+2], double Wmp_fim[I+2][J+2], double *delta_t){
...
int main(int argc, char *argv[]) {
	...
	//imprime resultados
	printf("\nNúmero de interações: %d ", inter-1);
